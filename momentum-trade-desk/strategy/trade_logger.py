...
LOT_SIZE = 75
DEFAULT_PREMIUM = 50  # ₹/share when broker fill unknown
HEDGE_PREMIUM = 8
THETA_CAPTURE = 0.20  # expected same-day decay fraction

NEUTRAL_BAND = 1500  # ₹ — |pnl| below this = NEUTRAL
...
def save_prediction(setup: dict[str, Any]) -> Path | None:
    """Save today's setup for next-day validation. Only QUALIFIED_SETUP."""
    ...
    path = PREDICTIONS_DIR / f"{pred_date}.json"
...
def _calc_pnl(
    ...
    low_spot: float,
    high_spot: float,
    premium_per_leg: float,
    lot_size: int,
    hedge_premium: float,
) -> dict[str, Any]:
    short_legs = [l for l in legs if "buy" not in l.get("instrument", "").lower()]
    n_short = len(short_legs)
    premium_in = n_short * premium_per_leg * lot_size
    hedge_cost = hedge_premium * lot_size

    pe_strikes = [l["strike"] for l in short_legs if l.get("type") == "PE"]
    ce_strikes = [l["strike"] for l in short_legs if l.get("type") == "CE"]
    ...
    if all_otm_close:
        pnl = premium_in * THETA_CAPTURE - hedge_cost * 0.7
        method = "theta_decay_all_otm"
    else:
...
def validate_pending(as_of: date | None = None) -> list[dict[str, Any]]:
    """
    Validate predictions from the previous trading day using today's session.
    Run at EOD on validate_date.
    """
    ...
    for path in sorted(PREDICTIONS_DIR.glob("*.json")):
        ...
        expected_validate = pred_date
        # Validate on first trading day AFTER prediction
        validate_date = pred_date
        while True:
            validate_date += timedelta(days=1)
            if validate_date.weekday() < 5:
                break
        ...
        hedge = pred["proposal"].get("tailHedge", {})
        hedge_strike = None
        if hedge.get("instrument"):
            parts = hedge["instrument"].split()
            for p in parts:
                if p.isdigit():
                    hedge_strike = int(p)
                    break
        ...
        pnl_info = _calc_pnl(
            ...
            low_spot=ohlc["low"],
            high_spot=ohlc["high"],
            premium_per_leg=pred.get("premiumPerLeg", DEFAULT_PREMIUM),
        ...
        row = {
            ...
            "legs_count": len(legs),
            "notes": (
                f"pred_spot={pred.get('spot')}; method={pnl_info['method']}; "
...
def _pending_predictions() -> list[dict[str, Any]]:
    ...
    for path in sorted(PREDICTIONS_DIR.glob("*.json"), reverse=True):
        with path.open(encoding="utf-8") as f:
            pred = json.load(f)
        if pred.get("validated"):
            continue
        ...
        pending.append(
            {
                "predictionDate": pred_date.isoformat(),
                "validateOn": validate_date.isoformat(),
                "spot": pred.get("spot"),
